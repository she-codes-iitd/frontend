...
function Grading(props) {
	const coureses = useRef([]);
	useEffect(() => {
		const getCourses = async () => {
			const res = await fetch("/courses/");
			const data = await res.json()
			coureses.current = data.courses;
	...
	return (
		<>
			...
			{
				coureses.current.map(course => {
					console.log(course);
					return (
						<div className="grid">
							<div className="item">
								<Link to={`/mentor/id/grading/${course._id}`} />
								<img src='https://image.freepik.com/free-vector/grades-concept-illustration_114360-697.jpg' alt='' />
								<div class="item__overlay">
									<h3 id="person1" aria-hidden="true">{course.name}</h3>
